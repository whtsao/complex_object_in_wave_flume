...
opts= Context.Options([
    ...
    ("cfl",0.75,"Desired CFL restriction"),
    ("he",0.35,"he relative to Length of domain in x"),
    ])
...
# TANK
tank = st.Tank2D(domain, dim=(3*wavelength, 1.75*water_level))

# SPONGE LAYERS
# generation zone: 1 wavelength
# absorption zone: 2 wavelengths
tank.setSponge(x_n=wavelength)


caisson = st.Rectangle(domain, dim=(13,1.39), coords=(0., 0.))
...
caisson.translate(np.array([1*wavelength, water_level+0.3]))

caisson2 = st.Rectangle(domain, dim=(13,1.39), coords=(0., 0.))
# set barycenter in middle of caisson
...
# translate caisson to middle of the tank
section_gap = 1.0
caisson2.translate(np.array([1*wavelength+13+section_gap, water_level+0.3]))
#caisson.rotate(0.174533)
...
free_x = np.array([0., 0., 0.]) # translational
...
body.setInertiaXX(np.array([300000., 3000000., 3000000.]))
# record values
...
#                        dragAlpha=dragAlpha)
#tank.BC['x-'].setConstantInletVelocity(U=np.array([.1, 0., 0.]), ramp=1, kk=0, dd=0 , b_or=np.array([-1., 0., 0.]))
#tank.BC['x-'].setTwoPhaseVelocityInlet(U=np.array([.1, 0., 0.]), waterLevel=water_level, smoothing=smoothing, Uwind=None,
#                                 vert_axis=1, air=1., water=0.,
#                                 kInflow=None, dissipationInflow=None,
#                                 kInflowAir=None, dissipationInflowAir=None)
#tank.BC['x+'].setHydrostaticPressureOutletWithDepth(seaLevel=water_level, rhoUp=rho_1, rhoDown=rho_0, g=g,
#                                              refLevel=water_level*2.0, smoothing=smoothing, orientation=None,U=None, Uwind=None,
#                                              pRef=0.0, vert_axis=1,
#                                              air=1.0, water=0.0,
#                                              kInflow=None, dissipationInflow=None,
#                                              kInflowAir=None, dissipationInflowAir=None)
#tank.BC['x+'].setConstantInletVelocity(U=np.array([.1, 0., 0.]), ramp=1, kk=0, dd=0 , b_or=np.array([1., 0., 0.]))
